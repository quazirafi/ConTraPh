...
final_acc_simd = -1.0
final_acc_target = -1.0

prev_max_num_correct = -1000

flag_15 = 0

for whole_exp in tqdm(range(fold_split)):
    ...
    cr = classification_report(total_label, total_pred, target_names=class_names)
    print(cr)
    # index = 0
    # for class_name, acc_i in acc.items():
    #     if index == 0:
    #         if acc_i > final_acc_simd:
    #             final_acc_simd = acc_i
    #     elif index == 1:
    #         if acc_i > final_acc_target:
    #             final_acc_target = acc_i
    #     index += 1
